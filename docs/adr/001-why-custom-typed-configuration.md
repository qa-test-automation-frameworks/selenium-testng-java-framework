...
## Decision
Use a small internal `FrameworkConfig` interface and `ConfigFactory` implementation. The loader reads defaults, optional `config.properties`, optional `${env}.properties`, an optional external `-Dconfig.file`, environment variables, and system properties in that override order.
